...
from nmp.policy.tanh_gaussian import TanhGaussianPointNetPolicy
...
# class PredictorPointNet(BaseProcessingNet):
#     def __init__(self, hp, input_size, output_size, num_layers=None, detached=False, spatial=True,
#                  final_activation=None, mid_size=None):
#         self.spatial = spatial
#         mid_size = hp.nz_mid if mid_size is None else mid_size
...
class EncoderPointNet(nn.Module):
    def __init__(self, hp):
        ...
        # self.net = PointNet(hp, hp.state_dim, hp.nz_enc, num_layers=hp.builder.get_num_layers())

        action_dim = hp.env.action_space.low.size
        obs_dim = hp.env.observation_space.spaces["observation"].low.size
        goal_dim = hp.env.observation_space.spaces["representation_goal"].low.size


        obstacles_dim = hp.env.obstacles_dim
        coordinate_frame = hp.env.coordinate_frame

        obstacle_point_dim = hp.env.obstacle_point_dim


        output_size = hp.nz_enc
        hidden_sizes = [hp.hidden_dim] * hp.n_layers
        robot_props = hp.env.robot_props
        elem_dim = obstacle_point_dim
        q_action_dim = 0
        input_indices =  hp.env.obs_indices

        self.net = PointNet(hidden_sizes=hidden_sizes,
                            robot_props=robot_props,
                            elem_dim=elem_dim,
