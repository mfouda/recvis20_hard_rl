...
class SACSkillPriorTrainer(TorchTrainer):
    def __init__(
        ...
        alpha=1,
        use_automatic_divergence_tuning=True,
        ...
        self.use_automatic_divergence_tuning = use_automatic_divergence_tuning
        if self.use_automatic_divergence_tuning:
    ...
    def train_from_torch(self, batch):
        ...
        if self.use_automatic_divergence_tuning:
            alpha_loss = -(
                self.alpha * (- policy_prior_divergence + self.target_divergence).detach()
            ).mean()
